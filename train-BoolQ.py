...
config_path = 'config/train_BoolQ.json'
...
df_train=pd.read_csv('/root/WIC/00-data/SKT_BoolQ_Train.tsv',sep='\t')
df_val=pd.read_csv('/root/WIC/00-data/SKT_BoolQ_Dev.tsv',sep='\t')
print(df_val.columns)
def T5_tokenizer(tokenizer, text, question, MAX_LEN):
...
for idx, data in tqdm(df_train.iterrows()):
    train_input_id, train_attention_mask = T5_tokenizer(tokenizer, data['Text'], data['Question'], MAX_LEN)
    
    train_input_ids.append(train_input_id)
    train_attention_masks.append(train_attention_mask)
    
    train_data_labels.append(data['Answer(FALSE = 0, TRUE = 1)'])
...
for idx, data in tqdm(df_val.iterrows()):
    val_input_id, val_attention_mask = T5_tokenizer(tokenizer, data['Text'], data['Question'], MAX_LEN)
    
    val_input_ids.append(val_input_id)
    val_attention_masks.append(val_attention_mask)
    val_data_labels.append(data['Answer(FALSE = 0, TRUE = 1)'])
...
#         print('val_acc {}, val_f1 {}, val_precision {}, val_recall {}'.format(val_acc/nb_val_steps, val_f1/nb_val_steps, val_precision/nb_val_steps, val_recall/nb_val_steps))
        ...
        if avg_val_acc > best_val_acc:
            ...
            # ... save weight..........
            print(avg_val_acc)
            torch.save(model, model_PATH + 'model_BoolQ_{}.pt'.format(config_path[7:-6]))
        else:
            early_stop_cnt +=1 
