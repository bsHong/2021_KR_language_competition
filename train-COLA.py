...
config_path = 'config/train_COLA.json'
...
df_train=pd.read_csv('/root/WIC/00-data/NIKL_CoLA_train.tsv', sep='\t')
df_val=pd.read_csv('/root/WIC/00-data/NIKL_CoLA_dev.tsv', sep='\t')

def T5_tokenizer(tokenizer, sentence, MAX_LEN):
...
for idx, data in tqdm(df_train.iterrows()):
    train_input_id, train_attention_mask = T5_tokenizer(tokenizer, data['sentence'], MAX_LEN)
    train_input_ids.append(train_input_id)
    train_attention_masks.append(train_attention_mask)
    
train_data_labels = df_train['acceptability_label']
val_input_ids =[]
val_attention_masks =[]
val_data_labels = []

for idx, data in tqdm(df_val.iterrows()):
    val_input_id, val_attention_mask = T5_tokenizer(tokenizer, data['sentence'], MAX_LEN)

    val_input_ids.append(val_input_id)
    val_attention_masks.append(val_attention_mask)
    
val_data_labels = df_val['acceptability_label']
...
#             loss = model.loss_function(*outputs)
    ...
    #         scheduler.step()
            ...
            tr_acc += matthews_corrcoef(true_list,preds_list)
        ...
        with torch.no_grad():
            ...
            for step, val_batch in enumerate(val_loader):
                ...
                val_acc += matthews_corrcoef(val_true_list,val_preds_list)
