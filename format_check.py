...
def typeCheck(eachTask, eachTaskPred):
	...
	for idx, eachPred in enumerate(eachTaskPred):
		...
		# index check
		taskIdxList.append(eachPred["idx"])

	# index check
	if eachTask in ["boolq", "wic", "copa"]:
		eachTaskGoldIdxList = list(range(taskGoldStartIdxDict.get(eachTask), taskGoldLenDict.get(eachTask)+1))
	else:
		eachTaskGoldIdxList = list(range(taskGoldStartIdxDict.get(eachTask), taskGoldLenDict.get(eachTask)))

	if eachTaskGoldIdxList != taskIdxList:
		print ("[ERROR]: index number mismatch in the sumitted \"{}\" task file.".format(eachTask))
		return False
