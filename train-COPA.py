...
config_path = 'config/train_COPA0.0001.json'
...
df_train=pd.read_csv('/root/WIC/00-data/SKT_COPA_Train.tsv',sep='\t')
df_val=pd.read_csv('/root/WIC/00-data/SKT_COPA_Dev.tsv',sep='\t')

# COPA
# token_1 : choice + quetion + context , 0
# token_2 : choice + quetion + context , 1
def T5_tokenizer(tokenizer, choice, sentence, question, MAX_LEN):
    cat = sentence + '</s>' + question
...
for idx, data in tqdm(df_train.iterrows()):
    ...
    train_input_id, train_attention_mask = T5_tokenizer(tokenizer, data['1'], data['sentence'], data['question'], MAX_LEN)
    input_ids.append(train_input_id)
    attention_masks.append(train_attention_mask)
    
    train_input_id2, train_attention_mask2 = T5_tokenizer(tokenizer, data['2'], data['sentence'], data['question'], MAX_LEN)
    
    input_ids.append(train_input_id2)
    attention_masks.append(train_attention_mask2)
    
    train_input_ids.append(input_ids)
    train_attention_masks.append(attention_masks)
    
    train_data_labels.append(data['Answer']-1)
...
for idx, data in tqdm(df_val.iterrows()):
    input_ids, attention_masks = [], []
    val_input_id, val_attention_mask = T5_tokenizer(tokenizer, data['1'], data['sentence'], data['question'], MAX_LEN)
    input_ids.append(val_input_id)
    attention_masks.append(val_attention_mask)
    
    val_input_id2, val_attention_mask2 = T5_tokenizer(tokenizer, data['2'], data['sentence'], data['question'], MAX_LEN)
    input_ids.append(val_input_id2)
    attention_masks.append(val_attention_mask2)
    
    val_input_ids.append(input_ids)
    val_attention_masks.append(attention_masks)
    val_data_labels.append(data['Answer']-1)
...
val_data_labels = torch.tensor(val_data_labels, dtype=torch.long)
...
model = model.T5_COPA.from_pretrained(model_name, **model_config)
...
#         print('val_acc {}, val_f1 {}, val_precision {}, val_recall {}'.format(val_acc/nb_val_steps, val_f1/nb_val_steps, val_precision/nb_val_steps, val_recall/nb_val_steps))
        ...
        if avg_val_acc > best_val_acc:
            ...
            # ... save weight..........
            print(avg_val_acc)
            torch.save(model, model_PATH + 'model_COPA.pt')
